...
public final class Farmworld extends JavaPlugin {
  ...
  public void onEnable() {
    ...
    String path = Bukkit.getServer().getPluginManager().getPlugin("WorldEdit").getDataFolder()
        .getAbsolutePath() + File.separator + "schematics" + File.separator
        + getConfig().getString("spawn-schematic");
    ...
    if (worldManager != null) {
      ...
      if (farmworldWorld == null) {
        worldManager.addWorld("farmworld", // The worldname
            World.Environment.NORMAL, // The overworld environment type.
            null, // The world seed. Any seed is fine for me, so we just pass null.
            WorldType.LARGE_BIOMES, // Nothing special. If you want something like a flat world,
                                    // change this.
      ...
      World world = farmworldWorld.getCBWorld();
      farmworldWorld
          .setSpawnLocation(new Location(world, 0, world.getHighestBlockAt(0, 0).getY() + 1, 0));
  ...
  public void placeSchematics(Clipboard clipboard, Location loc) {// Pasting Operation
    ...
    EditSession editSession =
        WorldEdit.getInstance().newEditSession(BukkitAdapter.adapt(loc.getWorld()));
    ...
    try { // This simply completes our paste and then cleans up.
      ...
      getServer().getLogger()
          .warning(ChatColor.RED + "OOPS! Something went wrong, please contact an administrator");
      e.printStackTrace();
    }
  }
  ...
  public void regenFarmworld() {
    ...
    World world = farmworldWorld.getCBWorld();
    placeSchematics(getClipboard(),
        new Location(world, 0, world.getHighestBlockAt(0, 0).getY() - 1, 0));
